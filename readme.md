...
В проекте есть автотесты (Django `TestCase`) для:
- кастомной модели `User` (создание, нормализация телефона, суперпользователь);
- регистрации, логина, валидации форм;
- редактирования профиля (валидация GitHub, нормализация телефона);
- AJAX-эндпоинтов (toggle-favorite, toggle-participate, complete);
- избранного и фильтрации пользователей.
...
В `.github/workflows/ci.yml` настроен GitHub Actions:
поднимает PostgreSQL, прогоняет `flake8` и `python manage.py test`
на каждый push/PR в `main`.

## Линтер

Конфиг flake8 — в `setup.cfg`. Лимит длины строки — 100 символов
(как разрешено в задании). Запуск:
```bash
